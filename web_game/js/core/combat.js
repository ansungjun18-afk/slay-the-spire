...
function calcDamage(baseDmg, attacker, defender) {
  const rawStrength = (attacker.buffs && attacker.buffs.strength) || 0;
  const strengthDown = (attacker.buffs && attacker.buffs.strength_down) || 0;
  const attackerStrength = rawStrength - strengthDown;
  let dmg = baseDmg + attackerStrength;
...
function takeDamage(target, amount) {
  ...
  if (target === game.player) {
    ...
    if(finalDmg > 0) {
      target.hp -= finalDmg;
      actualHpLoss = finalDmg;
    }
    log(`플레이어 피격! (데미지: ${finalDmg}, 방어도흡수: ${blocked}, 골골이흡수: ${ostyBlocked})`);
    
    if (actualHpLoss > 0) {
      game.player.timesLostHpThisCombat = (game.player.timesLostHpThisCombat || 0) + 1;
      game.player.lostHpThisTurn = (game.player.lostHpThisTurn || 0) + 1;
      if (game.battle && game.battle.isPlayerTurn) {
        if (target.buffs.rupture > 0) {
          target.buffs.strength = (target.buffs.strength||0) + target.buffs.rupture;
          log(`[파열] 힘 ${target.buffs.rupture} 획득!`);
        }
        if (target.buffs.inferno > 0) {
          log(`[불바다] 적에게 ${target.buffs.inferno} 피해!`);
          if (game.enemy) {
            let infDmg = calcDamage(target.buffs.inferno, game.player, game.enemy);
            let b = Math.min(infDmg, game.enemy.block || 0);
            game.enemy.block -= b; infDmg -= b;
            game.enemy.hp -= infDmg;
            log(`적에게 ${infDmg} 데미지!`);
          }
        }
      }
